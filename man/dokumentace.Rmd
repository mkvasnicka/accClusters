...
- tento offline neinteraktivní software \sw, který ze vstupních dat spočítá potřebné metriky, identifikuje místa častých nebo nebezpečných nehod a připraví podklady potřebné pro jejich interaktivní vizualizaci a
...
Použití dockerového kontejneru vyžaduje instalaci systému docker a dedikování dostatečných zdrojů pro dockerové kontejnery. Vlastní systém se pak stáhne automaticky při prvním použití při spuštění spouštěcího skriptu, viz následující oddíl.

Instalace aplikace v\ jazyce\ R vyžaduje několik kroků:
...
1. instalaci potřebných balíků v\ příslušných verzích
    - nainstalujte R\ balík `renv` a v\ R\ spusťte `renv::restore()`
    - v\ závislosti na operačním systému může vyžadovat instalaci mnoha systémových knihoven; na operačním systému Ubuntu 22.04 je možné tyto knihovny nainstalovat naráz pomocí skriptu obsaženého v\ souborou `Dockerfile`
...
- `-d <data folder>` umožňuje zadat jinou absolutní cestu ke složce s\ daty; implicitně se předpokládá, že data jsou uložena v\ aktuálním adresáři
...
Zbývající složka `gis` (vytvořená/aktualizovaná pouze v\ případě, že software identifikuje i\ klastry nehodových míst) slouží k\ volitelná vizualizaci klastrů nehodových míst v\ GISu, např.\ v\ QGISu.
