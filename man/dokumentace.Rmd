...
Tyto konstanty jsou dvou typů: První typ (označený spodním indexem\ 1) násobí *počet* škod v\ nehodě: např.\ $d_1$ se násobí počtem zemřelých v\ dané nehodě. Druhý typ (označený spodním indexem\ 2) násobí nejhorší škodu bez ohledu na počet. Uvažme např. nehodu, při které nikdo nezahynul ($D=0$), dva lidé se zranili těžce ($S=2$), tři lidé lehce ($L=3$) a nastala materiální škoda $M$. Váha této nehody pak bude

$$
2 \times s_1 + 3 \times l_1 + m_1 \times M + c + s_2,
$$

protože z\ koeficientů z\ první skupiny se zahrne dvakrát $s_1$ za dva těžce zraněné, třikrát $l_1$ za tři lehce zraněné a $m_1 \times M + c_1$ za materiální škodu; ze druhé skupiny se zahrne jedenkrát $s_2$, protože nejtěžší škoda je těžké zranění.

Všimněte si také rozdílu mezi $c$, $m_1$ a $m_2$:
...
- `UNIT_COST_DEAD` ... nezáporné číslo, váha každého smrtelného zranění ($d_1$),
- `UNIT_COST_SERIOUS_INJURY` ... nezáporné číslo, váha každého těžkého zranění ($s_1$),
- `UNIT_COST_LIGHT_INJURY` ... nezáporné číslo, váha každého lehkého zranění ($l_1$),
- `UNIT_COST_MATERIAL` ... nezáporné číslo, váha materiální škody v\ milionech\ Kč ($m_1$),
- `CONST_COST_DEAD` ... nezáporné číslo, paušální částka v\ mil. Kč, která se přidá ke každé nehodě, kde došlo k\ úmrtí ($d_2$),
- `CONST_COST_SERIOUS_INJURY` ... nezáporné číslo, paušální částka v\ mil.\ Kč, která se přidá ke každé nehodě, kde je nejhorší škoda těžké zranění ($s_2$),
- `CONST_COST_LIGHT_INJURY` ... nezáporné číslo, paušální částka v\ mil.\ Kč, která se přidá ke každé nehodě, kde je nejhorší škoda lehké zranění ($l_2$),
- `CONST_COST_MATERIAL` ... nezáporné číslo, paušální částka v\ mil.\ Kč, která se přidá ke každé nehodě, kde nedošlo k\ žádnému úmrtí ani zranění ($m_2$),
- `UNIT_COST_CONST` ... nezáporné číslo, konstanta připočítaná k\ hodnotě každé nehody ($c$),

Příklad nastavení, kde se hodnota smrtelného zranění nastaví na hodnotu 12\ milionů\ Kč, hodnota těžkého zranění čtvrtinu této částky, hodnota lehkého zranění na 3\ promile této částky a materiální škoda se zahrne ve své skutečné výši:
...
Otázkou je, na jakou výši tyto parametry nastavit. Jednou možností je nastavit parametr\ $d$ na hodnotu statistického života v\ milionech\ Kč, $s$ a $l$ na její poměrné zlomky a $m=1$ a $c=0$.
