...
- procesor minimálně třídy i5 (vyšší je lepší), minimálně 8\ jáder (větší počet jader je výhodou),
...
- `-p <docker run parameters>` umožnuje zadat volitelné parametry předané dockeru při spuštění kontejneru
...
Mapový podklad je k\ dispozici na Geoportálu ČÚZK. Jedná se o\ Soubor správních hranic a hranic katastrálních území ČR v\ projekti JTSK.
...
Většina přípravy dat probíhá paralelně na větším počtu jader. Parametry uvedné v\ tomto oddíle určují, kolik jader může být paralelně využito. Jsou zde dvě možnosti: Buď uživatel nastaví počet jader sám, nebo dovolí systému, aby počet jader odhadl. 
...
Intuitivně je klastr nehodových míst ta část silniční sítě, na níž se nachází mnoho nehod nebo nebezpečné nehody. Technícky se klastr nehodových míst hledá tak, že se hledají ty části silniční sítě, kde je hustota pravděpodobnosti nehody obzvlášť vysoká:
