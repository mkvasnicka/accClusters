...
Profily umožňují identifikovat klastry míst s\ častými/nebezpečnými nehodami podle několika kritérií současně. Pokud v\ adresáři `DIRORIGIN/config` existuje pouze soubor `config.R`, pak se vytvoří pouze jeden implicitní profil (s\ názvem `default`). Pokud zde existují soubory se jmény `profile_???.R`, kde znaky `???` označují písmena nebo číslice, pak se pro každý takový soubor vytvoří jeden profil (a\ implicitní profil nebude vytvořen). To např.\ znamená, že pokud

- existuje pouze `config.R`\ $\Rightarrow$ vytvoří se pouze jeden implicitní profil
- existuje `config.R` a `profile_cost.R`\ $\Rightarrow$ vytvoří se pouze jeden profil vytvořený ze souboru `profile_cost.R`
- existuje `config.R`, `profile_cost.R` a `profile_equal.R`\ $\Rightarrow$ vytvoří se dva profily, jeden z\ každého profilového souboru
...
Software pro přípravu dat nejdříve načte konfiguraci a všechny profily, zkontroluje je a uloží do souboru `profiles.rds`. Pokud cokoli změníte, přečtěte si prosím znovu oddíl\ \ref{sec:conf-one}. Ten vysvětluje, jak se konfigurace softwaru \sw{} interně zpracovává.
