...
V\ tomto oddílu popíšeme, jak systém přípravy dat funguje jako celek. Detaily vstupů, nastavení, výstupů apod. budou podrobněji vysvětleny v\ následujících oddílech.


## Obecný přehled

Po svém spuštění systém postupně provádí následující kroky:
...
1. spočítá hustoty pravděpodobnosti nehod/škod způsobených nehodami na celé silniční síti,
1. najde místa s\ výraznou pravděpodobností nehod/způsobených škod a vytvoří klastry takových míst a
1. jako bonus převede klastry nebezpečných úseků do formátu pro GIS.

Další krok se vždy začne provádět až po skončení předchozího kroku. Některé kroky (zejména kroky\ 1--3) jsou rychlé a provádí se na jednom jádru. Ostatní kroky se musejí provést zvlášť pro každý okres nebo kombinaci okresu, časového okna a různých alternativ nastavení systému (typicky vah jednotlivých nehod). Tyto kroky jsou paralelizované a běží na více jádrech systému\ -- vždy jeden okres nebo kombinace okresu, časového okna a parametrizace výpočtu na jednom jádře. Další krok se spustí vždy až ve chvíli, kdy doběhne výpočet pro všechny okresy apod.
...
Při každém spuštění systému je v\ adresáři `DIRORIGIN/log` vytvořen nový logový soubor se jménem `YYYY-MM-DD-hh-mm-ss.log`, kde `YYYY-MM-DD` je datum a `hh-mm-ss` je čas spuštění systému. Všechny kroky systému logují do tohoto souboru.
...
Při tvorbě binární konfigurace systém hledá v\ adresáři `DIRORIGIN/config` soubory `config.R` a `profile_???.R`, kde `???` jsou nějaké znaky anglické abecedy nebo číslice. Soubor `config.R` je povinný, soubory `profile_???.R` jsou volitelné.

Pokud je kterýkoli z\ existujících souborů novější než binární konfigurační soubor `profiles.rds`, binární konfigurace se aktualizuje. Tento systém funguje dobře v\ případě, že uživatel aktualizuje konfiguraci ve stávajících souborech nebo přidá nové profily. Pokud pouze smaže některý profilový soubor, k\ aktualizaci binární konfigurace nedojde. V\ takovém případě je třeba buď ručně smazat binární konfigurační soubor `profiles.rds` nebo aktualizovat datum modifikace kteréhokoli zdrojového konfiguračního souboru.
...
Data o\ nehodách se připravují ve dvou formách: 1)\ pro vizualizační část softwaru a 2)\ pro další výpočty. V\ obou případech jsou spojena data ze všech tabulek, které PČR do systému exportuje (viz\ oddíl\ \ref{sec:accidents}) a následně jsou rozdělena do jednotlivých souborů podle okresů.

V\ případě dat použitých pro identifikaci nehodových úseků jsou okresy zvětšeny o\ určitou vzdálenost a ze souboru jsou vyloučeny všechny nehody, které neleží dostatečně blízko u\ silniční sítě (nastavení viz\ oddíl\ \ref{kap:konfigurace}). Geolokace těchto nehod je zřejmě defektní a tyto nehody by do identifikace problémových úseků pouze zanesly šum.


## Podklady o silniční síti

Příprava podkladů o\ silniční síti probíhá v\ několika navazujících krocích:

1. Z\ celé mapy silniční sítě se vyfiltrují pouze podporované typy cest (vyhodí se lesní cesty apod.) a silnice se ořežou na jednotlivé okresy. Okresy jsou přitom zvětšeny o\ určitou vzdálenost na všechny strany (důvod je vysvětlen níže). K\ filtraci a ořezu silniční sítě se používá utilitka `osmium`.
1. Silniční síť se zjednoduší tak, aby byla zachovaná její topologie. V\ použitých OpenStreetMaps se totiž často silnice často skládají z\ velmi krátkých úseků, často o\ délce několika centimetrů. Takto krátké úseky jsou pospojovány do delších celků, pokud takto vzniklá silnice příliš nevybočí z\ tvaru skutečné silnice. V\ některých případech jsou spojeny i\ velmi blízké uzly nepodstatné pro topologii sítě.
...
## Hustoty pravděpodobnosti škod způsobených nehodami

Nejdůležitější i\ výpočetně nejnáročnější část hledání nebezpečných úseků spočívá ve výpočtu síťové jádrové hustoty nehod pro každý *lixel* silniční sítě reprezentovaný jeho středem. Síťové jádrové hustoty se počítají pomocí balíku **spNetwork** [@GelbRJournal; @GelbManual].

Jádrová hustota představuje jádrové vyhlazení (*smoothing*) bodového procesu na síti. Intuice je následující: Nad každou nehodou se na silniční síti nakreslí kopeček (*jádro*). (Buď mají všechny nehody kopečky stejně velké, nebo závažnější nehody mají větší kopečky než méně závažné nehody. Proces je komplikovanější o\ to, jak se "výška kopečků" rozděluje na křižovatkách.) Pak se v\ každém bodě sítě se sečte výška všech kopečků, jejichž nehoda je od daného bodu maximálně vzdálena o\ nějakou konstantu\ $b$ (*bandwidth*). V\ okolí bodu, kde je součet výšky kopečků vysoký, existuje více nehod (nebo jsou závažnější) než v\ okolí bodu, kde je tento součet nižší. Nebezpečné úseky silniční sítě jsou tedy charakterizovány vysokou jádrovou hustotou. (Pro techničtější popis konzultujte prosím např.\ @GelbRJournal.)

Systém umí spočítat hustoty pravděpodobnosti mnoha různých typů, viz oddíl\ \ref{sec:pardensities}. V\ nejjednodušším případě spočítá jádrovou hustotu, při které mají všechny nehody stejnou váhu. Ve složitějším mohou mít závažnější nehody vyšší váhu než méně závažné nehody. Systém také umožňuje stanovit délku\ $b$ nebo ji nechat adaptivně upravovat, viz\ @GelbManual. Umí také rozdělit "výšku kopečků" na křižovatkách nespojitě nebo spojitě, viz tamtéž.
...
Uživatel si ve webovém vizualizačním nástroji může stanovit, kolik promile nejhorších úseků se mu zobrazí, a\ o\ kolik *lixelů* se výsledné klastry zvětší. Klastry se tedy počítají pro takto zadané parametry $h$ (počet promile nejhorších úseků silniční sítě, které se mají najít) a $a$ (počet dodatečně přidaných *lixelů*):

1. Vybere se $h$ promile *lixelů* s\ nejvyšší hustotou pravděpodobnosti.
1. Idenitifikují se klastry takových *lixelů*, tj.\ skupiny lixelů, které tvoří souvislý graf (tj.\ z\ každého *lixelu* v\ klastru je možné přejet do jiného *lixelu* v\ klastru čistě přes úseky silniční sítě, které leží v\ klastru).
1. Ke každému klastru se rekurzivně $a\times$ přidají sousední lixely, které neleží v\ daném klastru.
...
Rozšíření klastrů v\ kroku\ 3


## Klastry nehodových úseků ve formátu pro GIS

Klastry úseků s\ častými nebo závažnými nehodami jsou exportovány do shapefilů, které je možné načíst v\ GISu. Způsob jejich použití je popsán v\ oddíle\ \ref{kap:gis}.
...
Další skupina parametrů ovlivňuje, jak se počítají hustoty pravděpodobností nehod, ze kterých se pak hledají problémová místa. Patří sem následující parametry:

- `NKDE_WEIGHTS` ... buď `"cost"` nebo `"equal"`, jaké váhy se přikládají jednotlivým nehodám
- `NKDE_METHOD` ... buď `"continuous"` nebo `"discontinuous"`, jak se počítají hustoty pravděpodobností: buď spojitě (`"continuous"`) nebo nespojitě (`"discontinuous"`); spojitý výpočet je poněkud přesnější, ale je také podstatně pomalejší
- `NKDE_ADAPTIVE` ... logická hodnota; zda je použito adaptivní vyhlazovací okno (`TRUE`), nebo ne (`FALSE`); při použijí adaptivního vyhlazovacího okna může být výpočet poněkud přesnější, ale je také podstatně pomalejší
- `NKDE_BW` ... kladné číslo, délka vyhlazovacího okna v\ metrech při použití neadaptivního vyhlazovacího okna (při `NKDE_ADAPTIVE = FALSE`)
- `NKDE_TRIM_BW` ... kladné číslo, maximální délka vyhlazovacího okna v\ metrech při použití adaptivního okna (když je `NKDE_ADAPTIVE = TRUE`)
- `NKDE_AGG` ... kladné číslo, vzdálenost v\ metrech: nehody, které jsou vzájemně blíže než je tato hodnota jsou při výpočtu zagregovány pro zrychlení výpočtu
