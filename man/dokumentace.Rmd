...
Software pro vizualizaci dopravních nehod má dvě části:

- tento offline neinteraktivní software, který vezme vstupní data, spočítá potřebné metriky, identifikuje místa častých nebo nebezpečných nehod a přípraví vstupy potřebné pro jejich interaktivní vizualizaci a
- interaktivní webovou aplikaci, která data vizualizuje; použití webové aplikace je popsané v\ separátním dokumentu.

Rozdělení softwaru do dvou částí je nutné kvůli výpočetní náročnosti přípravy dat, kterou není možné provádět "on-the-fly". Nejdříve se tedy pomocí této aplikace všechna data předpočítají, a\ pak se vizualizují pomocí webové aplikace.
...
Drtivá většina výpočtů běží paralelně na více jádrech. Čím větší počet jader má systém dedikovaný, tím rychleji příprava dat proběhne. Odhadovaný čas při prvním spuštění softwaru je při procesoru třídy i5 se čtyřmi jádry a 32 GB operační paměti zhruba 2--3\ dny. Zvýšení počtu jader tento čas proporcionálně zkrátí.

Výpočet běžící na jednom jádru vyžaduje typicky cca 4\ GB operační paměti (v\ některých krocích však 10\ GB). Větší počet jader se tedy použije pouze v\ případě, že je k\ dispozici dostatečné množství dedikované operační paměti.
...
Následující krok je spuštěn až ve chvíli, kdy doběhne celý předchozí krok. Je tomu tak proto, kdyby došlo z\ jakéhokoli důvodů k\ pádu nebo uživatelskému přerušení výpočtu pro některý okres.
