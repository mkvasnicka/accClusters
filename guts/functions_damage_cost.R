...
#
# inputs:
# - accidents ... (tibble) accidents table; it must include four columns:
...
add_damage_cost <- function(accidents, cost_function = unit_cost_function) {
    unit_cost <- cost_function()
    accidents |>
        mutate(accident_cost =
                   accident_dead * unit_cost["dead"] +
                   accident_serious_injury * unit_cost["serious_injury"] +
                   accident_light_injury * unit_cost["light_injury"] +
                   accident_material_cost * unit_cost["material_cost"] +
                   unit_cost["const"]
        )
}
